$(function(){
    $('#buscarpermiso').on('keyup', buscarpermisosnombre);
});
...
function buscarpermisosnombre(){
    var name = $("#buscarpermiso").val();
    $.when($.ajax({
        type: "post",
        url: '/permisos/buscar/'+name,
        data: {
            name: name
        },
        ...
        success: function (response) {
            ...
            response.forEach(element => {
                ...
                if ($('.permisoscheckgrabar'+element.id).is(':checked')) {
                    html=html+'<li id="li'+element.name+'" class="my-0 "><label class="my-0" id="3'+element.name+'"><input checked onclick="agregarpermiso('+element.id+',\''+element.name+'\',1)" class="name me-2 radioch radiocheck'+element.id+'" type="checkbox" value="'+element.id+'">'+element.name+'</label></li>';
                }else{
                    html=html+'<li id="li'+element.name+'" class="my-0 "><label class="my-0" id="3'+element.name+'"><input onclick="agregarpermiso('+element.id+',\''+element.name+'\',1)" class="name me-2 radioch radiocheck'+element.id+'" type="checkbox" value="'+element.id+'">'+element.name+'</label></li>';
                }
            });
            html=html+'</ul>';
